...
package com.bv.netpop.mobileQR.kotlin.barcodescanner
...
import com.bv.netpop.mobileQR.kotlin.VisionProcessorBase
import java.util.ArrayList

/** Barcode Detector Demo.  */
class BarcodeScannerProcessor(context: Context, bc: ArrayList<String>,projectAdapter: BarcodeActivityAdapter) : VisionProcessorBase<List<Barcode>>(context) {
  ...
  //     .build();
  private val barcodeScanner: BarcodeScanner = BarcodeScanning.getClient(BarcodeScannerOptions.Builder()
              .setBarcodeFormats(Barcode.FORMAT_QR_CODE)
              .build())

  private var bc: ArrayList<String> = bc
  private var projectAdapter: BarcodeActivityAdapter = projectAdapter
  ...
  override fun onSuccess(barcodes: List<Barcode>, graphicOverlay: GraphicOverlay) {
    ...
    for (i in barcodes.indices) {
      ...
      val rawBC = barcode.rawValue.toString()
